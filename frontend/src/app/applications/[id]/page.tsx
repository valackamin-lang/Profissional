...
export default function ApplicationDetailPage() {
  ...
  const loadApplication = async () => {
    try {
      ...
      // Redirect to the full URL with jobId for better UX
      if (app?.job?.id) {
        router.replace(`/jobs/${app.job.id}/applications/${applicationId}`);
      }
    } catch (error: any) {
              ...
              <div className="flex items-center space-x-3 mb-4">
                ...
                <div>
                  ...
                  {application.job?.id ? (
                    ...
                    <p className="text-lg font-semibold text-gray-900">{application.job.title}</p>
                  )}
